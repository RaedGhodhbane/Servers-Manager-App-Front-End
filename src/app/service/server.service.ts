...
export class ServerService {
  ...
  // server$ : means it's an observable
  ...
  save$ = (server: Server) => <Observable<CustomResponse>>
    this.http.post<CustomResponse>(`${this.apiUrl}/server/save`, server)
      // NB : server is the body of the request
      .pipe(
        tap(console.log),
        catchError(this.handleError)
      );
  ...
  //               .filter((server: { status: any; }) => server.status === status).length > 0 ? `Servers filtered by
  //         ${status === Status.SERVER_UP ? 'SERVER UP'
  //               : 'SERVER DOWN'} status` : `No servers of ${status} found`,
  //             data: {
  //               servers: response.data.servers
  //                 .filter((server: { status: any; }) => server.status === status)
  //             }
  //           }
  //       );
  //       subscriber.complete();
        ...
        subscriber => {
            ...
            if (response.data && response.data.servers) {
                ...
                subscriber.next({
                    ...
                    data: { servers: [] } // or any default value
                });
            }
            subscriber.complete();
        }
